...
class PizzariaAPPSupabaseUser extends BaseAuthUser {
  PizzariaAPPSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pizzariaAPPSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PizzariaAPPSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
